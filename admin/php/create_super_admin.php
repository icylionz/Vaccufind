...
if(isset($_POST['create_super_admin_btn']))
{
    session_start();
    ...
    $firstname = validate($_POST['super_admin_FirstName']);
    $lastname = validate($_POST['super_admin_LastName']);
    $username = validate($_POST['super_admin_Username']);
    $password = validate($_POST['super_admin_pass']);
    $password2 = validate($_POST['super_admin_pass2']);
    ...
    {
        header("Location: ../super_admin_console.php?error=Super Admin Username is required");
        exit();
    }
    else if (strlen($username) < 8 || strlen($username) > 30 )
    {
        header("Location: ../super_admin_console.php?error=Super Admin Username must be between length of 8-30");
        exit();
    }
    ...
    else if($password == $password2)
    {
        ...
        $supersql = "INSERT INTO superadminlogin(superAdminFirstName, superadminLastName, username, passwrd) 
                    VALUES('$firstname', '$lastname', '$username', '$password' )";
